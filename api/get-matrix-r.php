...
try {
    ...
    // =========================================================================
    // LANGKAH 5: Lakukan Normalisasi (Membuat Matriks R)
    // =========================================================================
    $normalizedMatrix = [];
    foreach ($alternativesMatrix as $altId => $altData) {
        ...
        foreach ($criterias as $critId => $critData) {
            ...
            $critCode = $critData['code']; // Ambil kode kriteria (C1, C2, dst.)
            ...
            // Langsung masukkan ke hasil dengan key kode kriteria
            $normalizedRow[$critCode] = round($normalizedValue, 4);
        }
        $normalizedMatrix[] = $normalizedRow;
    }

    // =========================================================================
    // LANGKAH 6: Kirim respon JSON berisi Matriks Normalisasi (R)
    // =========================================================================
    echo json_encode([
        'status' => 'success',
        'message' => 'Matriks Normalisasi (R) berhasil dibuat',
